...
class UserModel {
    ...
    static async banUser(userId, reason) {
      ...
      WHERE id = $1
    `;
        await database_1.pool.query(query, [userId, reason]);
    }
     ...
     */
    static async incrementReviewCount(userId) {
        const query = `
      UPDATE users 
     ...
     * Update trust score
     */
    static async updateTrustScore(userId, score) {
        const query = `
      UPDATE users 
      SET trust_score = $2
      WHERE id = $1
    `;
        await database_1.pool.query(query, [userId, score]);
    }
}
